...
const sendEmail = async (options: EmailOptions): Promise<boolean> => {
  ...
  try {
    const mailOptions: nodemailer.SendMailOptions = {
      from: `"${config.smtpUser}" <${config.smtpUser}>`,
      to: options.to,
 ...
 * @returns 发送结果
 */
const sendVerificationEmail = async (email: string, code: string, type: string = '注册'): Promise<boolean> => {
 ...
 * @returns 发送结果
 */
const sendPasswordResetEmail = async (email: string, resetLink: string): Promise<boolean> => {
  const subject = '密码重置';
  const html = `
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
 ...
 * @returns 发送结果
 */
const sendOrderConfirmationEmail = async (email: string, orderData: OrderData): Promise<boolean> => {
  const subject = '订单确认';
  const html = `
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
...
const sendSMS = async (phone: string, message: string): Promise<boolean> => {
  if (!config.smsAccessKey || !config.smsAccessSecret) {
 ...
 * @returns 发送结果
 */
const sendSMSCode = async (phone: string, code: string, type: string = '注册'): Promise<boolean> => {
 ...
 * @returns 发送结果
 */
const sendOrderStatusSMS = async (phone: string, orderData: OrderStatusData): Promise<boolean> => {
