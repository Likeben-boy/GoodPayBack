...
export const cartValidation = {
  addToCart: [
    body("productId").isInt({ min: 1 }).withMessage("商品ID必须是大于0的整数"),
    body("quantity")
      .isInt({ min: 1, max: 99 })
      .withMessage("数量必须是1-99之间的整数"),
    body("specs").optional().isObject().withMessage("规格必须是对象"),
    body("notes")
      .optional()
      .trim()
      .isLength({ max: 200 })
  ...
  updateCartItem: [
    param("id").isInt({ min: 1 }).withMessage("购物车项ID必须是大于0的整数"),
    body("quantity")
      .isInt({ min: 1, max: 99 })
      .withMessage("数量必须是1-99之间的整数"),
  ],
  ...
  batchRemoveCartItems: [
    body("itemIds").isArray({ min: 1 }).withMessage("itemIds必须是非空数组"),
    body("itemIds.*")
      .isInt({ min: 1 })
  ...
  selectCartItem: [
    param("id").isInt({ min: 1 }).withMessage("购物车项ID必须是大于0的整数"),
    body("selected").isBoolean().withMessage("selected必须是布尔值"),
  ],
...
export const orderValidation = {
  createOrder: [
    body("addressId").isInt({ min: 1 }).withMessage("地址ID必须是大于0的整数"),
